...
XRate is a Chrome Manifest V3 extension that automatically detects prices on webpages and displays inline currency conversions.

**Core architecture:**
- **Background service worker** (`background.js`) — fetches exchange rates from open.er-api.com API (ExchangeRate-API supporting 160+ currencies including Middle Eastern) and caches them in `chrome.storage.local` with 1-hour TTL
- **Content script** (`content.js`) — injected into all pages; uses TreeWalker + regex to detect prices in text nodes, computes conversions, and injects styled `<span>` badges
- **Popup UI** (`popup.html/js/css`) — extension settings interface for toggling on/off, selecting target currency, and manually refreshing rates
- **Shared currency data** (`currencies.js`) — loaded by both content script (via manifest) and background worker (via `importScripts`)
...
.\pack.ps1
```
Creates `CurrencyLens.zip` containing only the files needed for distribution. Verifies all required files exist before packaging.

### Local testing (sideload)
1. Open Chrome → `chrome://extensions`
2. Enable **Developer mode**
3. Click **Load unpacked** → select `C:\PersonaProjects\CurrencyLens`
...
2. Regex matches four price formats: `$100`, `100€`, `USD 100`, `100 USD`
...
- Rates cached with `{ rates: {USD: 1, EUR: 0.92, …}, date: "2024-01-15", fetchedAt: 1234567890 }`
...
- `cl_settings.enabled` (boolean) — master on/off toggle
- `cl_settings.targetCurrency` (string) — ISO 4217 code (default: "EUR")
- `cl_rates` (object) — cached rate data with `fetchedAt` timestamp
...
- **Message passing:** All communication between background ↔ content ↔ popup uses `chrome.runtime.sendMessage` / `onMessage`
...
- **background.js** — Rate fetching logic, settings management, message router
- **content.js** — DOM scanning, price detection regex, conversion math, badge injection
- **currencies.js** — Shared data structure for symbols (`$`, `€`), multi-char symbols (`CA$`, `R$`), ISO codes, and currency metadata (name, flag, decimals)
- **popup.html/js/css** — 300×400px UI for settings (toggle, currency dropdown, refresh button)
- **content.css** — Badge styling with dark mode support via `prefers-color-scheme`
- **pack.ps1** — PowerShell script to create Web Store ZIP package
- **generate-icons.html** — Standalone HTML tool to generate 16/48/128px PNGs from canvas

## Deployment

### Publishing checklist
1. Run `.\pack.ps1` to create `CurrencyLens.zip`
2. Icons must exist in `icons/` (use `generate-icons.html`)
3. Upload to [Chrome Web Store Developer Dashboard](https://chrome.google.com/webstore/devconsole) — $5 one-time fee
4. Same ZIP works for [Microsoft Edge Add-ons](https://partner.microsoft.com/dashboard/microsoftedge) — no fee
...
- Arabic currency symbols supported: د.إ, ر.س, ر.ق, د.ك, د.ب, ر.ع, د.ع
