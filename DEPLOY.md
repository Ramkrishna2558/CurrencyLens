...
2. Create a new repository named **CurrencyLens**
3. **Do NOT** initialize with README, .gitignore, or license (we already have these)
4. Click **Create repository**
5. Copy the HTTPS or SSH URL shown (e.g., `https://github.com/YOUR_USERNAME/CurrencyLens.git`)
6. Run these commands:

```powershell
cd C:\PersonaProjects\CurrencyLens
...
```powershell
cd C:\PersonaProjects\CurrencyLens
gh repo create CurrencyLens --public --source=. --remote=origin --push
```
...
- Click **Load unpacked** → select `C:\PersonaProjects\CurrencyLens`
...
- Upload the same `CurrencyLens.zip`
...
1. **README.md** — Change the open source link:
   ```markdown
   Open source: https://github.com/YOUR_USERNAME/CurrencyLens
   ```

2. **store/description.txt** — Same link update
